...
object FixedSearchUtils {
    fun resolvePhpClass(e: AnActionEvent): PhpClass? {
        ...
        if (psiElement is PhpClass) {
            ProjectLogHelper.log(project, "FixedSearchUtils.resolvePhpClass: use contextElement direct=${psiElement.fqn}")
            return psiElement
        }
        ...
        if (fromContext != null) {
            ProjectLogHelper.log(project, "FixedSearchUtils.resolvePhpClass: from context parent=${fromContext.fqn}")
            return fromContext
        }
        ...
        if (psiFile != null && psiFile.name.endsWith(".php", ignoreCase = true)) {
            ...
            if (classInFile != null) {
                ProjectLogHelper.log(project, "FixedSearchUtils.resolvePhpClass: from file firstClass=${classInFile.fqn}")
                return classInFile
            } else {
                ProjectLogHelper.log(project, "FixedSearchUtils.resolvePhpClass: psiFile present but no PhpClass in file=${psiFile.name}")
            }
        }
        ...
        try {
            ...
            ApplicationManager.getApplication().invokeLater {
                if (filteredStaticUsages.isNotEmpty()) {
                    FixedSearchUtils.showUsagesWithFallback(project, filteredStaticUsages, "固定搜索-静态调用")
                } else {
                    FixedSearchUtils.notifyInfo(project, "未找到静态方法调用")
                }
            }
        } catch (ex: Throwable) {
            ProjectLogHelper.log(project, "固定搜索-静态调用: 搜索异常 ${ex.message}")
            ApplicationManager.getApplication().invokeLater {
                FixedSearchUtils.notifyError(project, "搜索静态方法调用失败: ${ex.message}")
            }
        }
    }
